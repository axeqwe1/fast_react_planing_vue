    ...
    <div class="bg-surface-0 dark:bg-surface-900 p-6 shadow rounded-2xl flex flex-col gap-4">
      <div class="flex gap-4">
        <div class="flex flex-col gap-2 flex-1">
          ...
          >
            ...
            <Column field="capMP" header="MP Cap">
              <template #body="{ data }">
                <div class="flex items-center gap-2">
                  {{ data.capMP || 0 }}
                  <Button
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <span v-if="selectedItem"> Are you sure you want to delete this manpower config? </span>
        </div>
        <template #footer>
...
const selectedItem = ref<manualMP | null>(null)
const selectedIndex = ref<number>(-1)
const editingItem = ref<manualMP | null>(null)
...
function openDateEditor(item: manualMP, field: 'startDate' | 'endDate') {
...
async function toggleActive(item: manualMP) {
  item.isActive = !item.isActive
  const res = await ToggleActive(item.id)
  if (res.status === 200) {
...
function openMpCapEditor(item: manualMP) {
  editingItem.value = item
  tempMpCap.value = item.capMP || 0
  mpCapDialog.value = true
}

function saveMpCapEdit() {
  if (editingItem.value) {
    editingItem.value.capMP = tempMpCap.value
    UpdateData(editingItem.value)
  }
...
function confirmDelete(item: manualMP, index: number) {
